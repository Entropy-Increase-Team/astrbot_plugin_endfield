...
✅ **数据查询** - 便签、理智、干员面板、危机合约、抽卡记录/分析、全服统计、成就、日历
...
| `地区建设` / `建设` | 地区开发进度与调度券 |
| `成就列表` / `成就` | 成就达成情况 |
| `危机合约` / `危机` | 查询当前危机合约分数、任务和挑战记录 |
| `合约 [名称或ID]` | 指定危机合约名称或 ID 查询 |
| `危机 指标` / `危机 词条` | 查看完整危机指标/词条列表 |
...
### 3.0.3 (2026-08-24)
- 🧹 失效绑定：Framework Token 失效或会话过期时自动删除对应绑定，并尽力同步清理服务端记录
- 🔇 后台日志：同一失效 Token 只处理一次，避免理智推送持续刷出重复错误

### 3.0.2 (2026-07-02)
- 🗜️ 资源体积：压缩预览图与背景图，插件包体积降至 AstrBot 16MB 限制以内
- 🧹 发布清理：移除缓存文件，保留 HarmonyOS 字体文件不变

### 3.0.1 (2026-07-01)
- 🎨 危机合约渲染：移除参考 App UI 的返回、问号、重载控件，改为插件报告顶部信息条
- 🧭 指令引导：图片与帮助菜单补充 `/危机 指标`、`/合约 [名称或ID]` 使用提示

### 3.0.0 (2026-07-01)
- ✨ 危机合约：新增 `危机合约` / `危机` / `合约 [名称或ID]` 查询
- 📊 危机指标：新增 `危机 指标` / `危机 词条` 完整指标视图
- 🎨 渲染模板：新增红黑风格危机合约图片，展示最高评分、任务、奖章、挑战记录与队伍头像
- 🔌 API 适配：接入最新 `/api/endfield/card/crisis-contract`，自动从 `card/detail` 读取 `contractId`
